...
class StateMachine:
    ...
    def EnCaminoAModula(self, coordenadas, nuevoestado):
        robot.auto_recharge.publish(0)
        listener = tf.TransformListener()
        l = len(coordenadas)
        wait = True
        for i in range (0,l):
            if wait == True:
                robot.sendGoal2(coordenadas[i])
                self.mission_status.publish(self.estatus_mision)
                wait = False
                if self.estatus_mision == GoalStatus.SUCCEEDED:
                    wait =True
        rospy.sleep(3)
        if robot.estatus_mision == GoalStatus.SUCCEEDED:
            robot.estadoActual = nuevoestado
        ...
        while not rospy.is_shutdown():
            ...
            if robot.batterypercentage >= 50:
                if robot.estadoActual == EstadosSMART.Reset:
                    ...
                    robot.EnCaminoAModula(Coordenadas.Modula, EstadosSMART.RecogiendoEnModula)
